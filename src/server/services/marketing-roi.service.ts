...
export function getMarketingRoiData(
  timeRange: string,
  region: string,
): { weeks: WeeklyROI[]; totalSpend: number; totalRevenue: number; avgRoi: number } {
  ...
  const avgRoi = parseFloat(((totalRevenue / totalSpend - 1) * 100).toFixed(1));
