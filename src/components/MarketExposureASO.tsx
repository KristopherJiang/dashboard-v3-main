...
const FancyKPI = ({ label, value, change, isUp, icon: Icon, colorClass }: any) => (
  <div className="group relative flex-1 bg-white/60 backdrop-blur-md rounded-2xl p-3 border border-slate-200/50 shadow-sm transition-all duration-300 hover:shadow-md">
    <div
      className={`absolute -top-px left-4 w-6 h-[2px] bg-gradient-to-r from-transparent ${colorClass} to-transparent opacity-40`}
    ></div>
    ...
    <div className="flex items-end justify-between">
      ...
      <div
        className={`flex items-center text-[9px] font-black px-1.5 py-0.5 rounded-full ${isUp ? 'text-emerald-400 bg-emerald-50' : 'text-rose-500 bg-rose-50'}`}
      >
        {isUp ? '↑' : '↓'}
        {change}%
      </div>
    ...
    <div className="flex flex-col h-full rounded-2xl animate-in fade-in duration-300 gap-4">
      ...
      <div className="flex-1 overflow-y-auto min-h-0 flex flex-col gap-4">
        ...
        <div className="bg-white border border-slate-200 p-4 rounded-xl shadow-sm">
          <SectionHeader title="关键词覆盖与排名" icon={Search} />
            ...
            <div className="bg-slate-50 border border-slate-100 rounded-2xl p-3">
              ...
              <div className="flex items-baseline gap-2">
                <span className="text-xl font-black text-slate-800 tracking-tighter">
                  {kpiData.totalKeywords.value}
                </span>
                <span className="text-[9px] font-bold text-cyan-500">
                  +{kpiData.totalKeywords.change}
                </span>
              </div>
            </div>
            <div className="bg-slate-50 border border-slate-100 rounded-2xl p-3">
              ...
              <div className="flex items-baseline gap-2">
                <span className="text-xl font-black text-slate-800 tracking-tighter">
                  {kpiData.top3Keywords.value}
                </span>
                <span className="text-[9px] font-bold text-blue-500">
                  +{kpiData.top3Keywords.change}
                </span>
              </div>
            </div>
          </div>
                    ...
                    if (active && payload && payload.length) {
                      ...
                      return (
                        <div className="bg-slate-900 rounded-xl p-4 shadow-xl z-50 min-w-[120px]">
                          ...
                          <p className="text-slate-400 text-[9px] font-bold mb-3 pb-2 border-b border-white/10 uppercase tracking-widest">
                            关键词排名分布
                          </p>
                          <div className="flex flex-col gap-2">
                            {sortedPayload.map((entry: any, index: number) => (
                              <div
                                key={index}
                                className="flex justify-between items-center gap-4 text-[10px]"
                              >
