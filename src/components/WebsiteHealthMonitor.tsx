...
const COUNTRIES = [
  {
    ...
    packetLoss: '0.5%',
    iosStatus: 'normal',
    androidStatus: 'normal',
    iosDesc: 'App Store 正常访问',
    androidDesc: 'Google Play 正常访问',
  },
  {
    id: 'TH',
    flag: '🇹🇭',
...
const WebsiteHealthMonitor = () => {
  ...
  const getHealthStatus = (node: (typeof COUNTRIES)[0]) => {
    if (platform === 'app' && (node.iosStatus === 'removed' || node.androidStatus === 'removed'))
  ...
  return (
    <div className="h-full bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden relative flex flex-col">
      {/* 背景氛围层 */}
      <div
        className={`absolute top-0 left-0 w-full h-40 transition-colors duration-1000 opacity-5 ${currentStatus.bg}`}
      ></div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2.5">
                ...
                <div
                  className="flex items-center gap-1.5 px-2 py-0.5 rounded bg-emerald-50 border border-emerald-100 shadow-sm"
                  title="此板块为全局实时监测，不受历史时间轴影响"
                >
                  <span className="relative flex h-1.5 w-1.5">
                    <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                    <span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-emerald-500"></span>
                  </span>
                  <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest leading-none mt-px">
                    Live
                  </span>
                </div>
              </div>
      ...
      <div className="relative z-10 flex-1 p-6 flex flex-col">
        ...
        <div className="relative w-full flex-1 min-h-[14rem] bg-slate-50/50 rounded-2xl border border-slate-100 mb-4 overflow-hidden flex items-center justify-center">
          ...
          {/* Connecting Lines */}
          <svg className="absolute inset-0 w-full h-full pointer-events-none z-0"></svg>
          ...
          {COUNTRIES.map((country) => {
            ...
            const isAppRemoved =
              platform === 'app' &&
              (country.iosStatus === 'removed' || country.androidStatus === 'removed');
            const isBothRemoved =
              platform === 'app' &&
              country.iosStatus === 'removed' &&
              country.androidStatus === 'removed';
            ...
            return (
              ...
              >
                ...
                {platform === 'web' && country.lat >= 2500 && (
                  <div
                    className={`absolute bottom-[18px] left-[7px] -translate-x-1/2 px-2 py-0.5 rounded-full text-[9px] font-bold whitespace-nowrap shadow-sm text-white ${
                ...
                />
                <div
                  className={`text-[10px] font-bold tracking-wide break-keep whitespace-nowrap transition-colors ${isSelected ? 'text-slate-900 drop-shadow-md' : 'text-slate-600 drop-shadow-sm group-hover:text-slate-800'}`}
                >
                  {country.name}
                </div>
              </div>
                ...
                {platform === 'web' ? (
                  <>
                    <div className="flex justify-between items-center text-[10px] font-bold">
                      ...
                      <span className="text-emerald-400 flex items-center gap-1">
                        <div className="w-1.5 h-1.5 rounded-full bg-emerald-400"></div> 正常
                        (Online)
                      </span>
                    </div>
                    <div className="flex justify-between items-center text-[10px] font-bold">
                      ...
                      <span
                        className={
                          hoveredCountry.lat < 2500
                            ? 'text-emerald-400'
                            : hoveredCountry.lat < 4000
                              ? 'text-amber-400'
                              : 'text-rose-400'
                        }
                      >
                        {hoveredCountry.lat}ms
                      </span>
                    </div>
                    <div className="flex justify-between items-center text-[10px] font-bold">
                      <span className="text-slate-400">SLA 评估</span>
                      <span
                        className={hoveredCountry.lat < 2500 ? 'text-emerald-400' : 'text-rose-400'}
                      >
                  ...
                  <>
                    <div className="flex justify-between items-center text-[10px] font-bold">
                      ...
                      <span
                        className={
                          hoveredCountry.iosStatus === 'normal'
                            ? 'text-emerald-400'
                            : 'text-rose-400'
                        }
                      >
                    ...
                    <div className="flex justify-between items-center text-[10px] font-bold mt-2">
                      <span className="text-slate-400 flex items-center gap-1">
                        <Smartphone size={10} /> Google Play
                      </span>
                      <span
                        className={
                          hoveredCountry.androidStatus === 'normal'
                            ? 'text-emerald-400'
                            : 'text-rose-400'
                        }
                      >
        ...
        {platform === 'web' ? (
            ...
            <div className="text-right">
              <div className="flex gap-6 items-end">
                ...
                <div>
                  ...
                  <div
                    className={`text-base font-black leading-none h-[24px] flex items-center ${currentStatus.textLight}`}
                  >
                    {displayNode.lat}ms
                  </div>
                </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            {/* Google Play Card */}
            <div
              className={`bg-white rounded-3xl p-6 shadow-sm border border-slate-100 flex flex-col items-center justify-center relative transition-all group hover:shadow-md h-[160px]`}
            >
              <div
                className={`w-14 h-14 rounded-2xl bg-slate-50 flex items-center justify-center mb-3 shadow-inner`}
              >
                <img
              ...
              <h3 className="text-sm font-bold text-slate-900 mb-2 font-sans tracking-tight">
                {displayNode.name} Google Play
              </h3>
              <div
                className={`px-3 py-1 rounded-full text-[10px] font-bold ${displayNode.androidStatus === 'removed' ? 'bg-rose-50 text-rose-500 border border-rose-100' : 'bg-emerald-50 text-emerald-500 border border-emerald-100'}`}
              >
            ...
            >
              ...
              <h3 className="text-sm font-bold text-slate-900 mb-2 font-sans tracking-tight">
                {displayNode.name} Apple Store
              </h3>
              <div
                className={`px-3 py-1 rounded-full text-[10px] font-bold ${displayNode.iosStatus === 'removed' ? 'bg-rose-50 text-rose-500 border border-rose-100' : 'bg-emerald-50 text-emerald-500 border border-emerald-100'}`}
              >
        ...
        {platform === 'web' ? (
          <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 flex flex-col gap-4 mb-4 mt-auto">
            <div className="flex justify-between items-end">
              <div>
                ...
                <div className="text-2xl font-black text-slate-900 leading-none">
                  {Math.round(
                    (COUNTRIES.filter((c) => c.lat < 2500).length / COUNTRIES.length) * 100,
                  )}
                  %
                  <span className="text-sm font-bold text-slate-400 ml-2 tracking-tight">
                    全球地区达标
                  </span>
                </div>
              </div>
            ...
            <div className="w-full h-3 bg-slate-50 rounded-full flex overflow-hidden border border-slate-100">
              ...
              <div
                ...
                style={{
                  width: `${(COUNTRIES.filter((c) => c.lat < 2500).length / COUNTRIES.length) * 100}%`,
                }}
              ></div>
              {/* 需优化 */}
              <div
                ...
                style={{
                  width: `${(COUNTRIES.filter((c) => c.lat >= 2500 && c.lat <= 4000).length / COUNTRIES.length) * 100}%`,
                }}
              ></div>
              {/* 较差 */}
              <div
                ...
                style={{
                  width: `${(COUNTRIES.filter((c) => c.lat > 4000).length / COUNTRIES.length) * 100}%`,
                }}
              ></div>
            </div>
            ...
            <div className="flex justify-between items-center pt-1">
              <p className="text-[11px] font-bold text-slate-400 italic">
                基于全球 {COUNTRIES.length} 个核心骨干节点探测
              </p>
          ...
          <div className="bg-white rounded-3xl p-6 mb-4 shadow-sm border-2 border-emerald-400/60 relative overflow-hidden">
            ...
            <p className="text-[11px] font-bold text-slate-400 mb-6 tracking-wide">
              异常点位 (iOS/安卓) / 总监控点位
            </p>
