      ...
      retail: {
        ...
        children: [
            ...
            subChildren: [
              {
                key: 'ASA',
                label: 'ASA',
                val: Math.round(paidAdsVal * 0.42),
                pct: '42%',
                isSub: true,
              },
              {
                key: 'Google Ads',
                label: 'Google Ads',
                val: Math.round(paidAdsVal * 0.38),
                pct: '38%',
                isSub: true,
              },
              {
          ...
          { key: 'KOL', label: 'KOL', val: Math.round(total * kolPct), pct: '18%', icon: User },
          {
            key: 'Organic',
            label: 'Organic',
            val: Math.round(total * organicPct),
            pct: '16%',
            icon: Leaf,
          },
      ...
      <div className="flex-1 p-6 flex flex-col gap-6 overflow-y-auto">
        ...
        <div className="flex gap-[2px] rounded overflow-hidden mt-6 mb-4">
          ...
          <div
            ...
            style={{ flex: `${data.retail.val} 1 0%` }}
          >
            {/* Upper Main Stats */}
            <div className="bg-[#5a8c61] p-6 flex flex-col justify-center min-h-[160px]">
              ...
              <div className="text-base font-medium text-white/90">
                {data.retail.val.toLocaleString()}
              </div>
            </div>
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-center mb-1.5 gap-1">
                      <div
                        className="text-[13px] font-bold text-slate-800 tracking-wide leading-tight"
                    ...
                    <div className="flex flex-col">
                      <div className="text-[13px] font-bold text-slate-800 mb-0.5 leading-none">
                        {child.pct}
                      </div>
                      <div className="text-[13px] font-medium text-slate-800/90 leading-none">
                        {child.val.toLocaleString()}
                      </div>
                    </div>
                  </div>
                );
          ...
          <div
            className="bg-[#222a35] p-6 flex flex-col justify-center relative group transition-colors hover:bg-[#2a3441] min-w-0"
            style={{ flex: `${data.ib.val} 1 0%` }}
          >
        ...
        <div className="bg-[#fafcfb] p-5 rounded-xl border border-[#e0ebe3] flex items-start gap-3 text-[13px] text-[#4f7b58] font-medium leading-relaxed transition-all duration-300">
          ...
          {drilledBlock === 'Paid Ads' ? (
            <span className="animate-in fade-in duration-500">
              结构洞察：Paid Ads 共贡献{' '}
              {Math.floor(
                data.retail.children.find((c: any) => c.key === 'Paid Ads')?.val || 0,
              ).toLocaleString()}{' '}
              名新增用户。其中 <strong>ASA</strong> 渠道表现最为强劲，转化占比达 42%（
              {Math.floor(
                (data.retail.children.find((c: any) => c.key === 'Paid Ads')?.val || 0) * 0.42,
              ).toLocaleString()}
              人），<strong>Google Ads</strong> 稳定输出占比 38%。
            </span>
          ) : (
            <span className="animate-in fade-in duration-500">
              结构洞察：Retail 贡献 {data.retail.formatPct} 的新增，其中付费广告占比最高（
              {data.retail.children.find((c: any) => c.key === 'Paid Ads')?.pct || '0%'}），KOL 贡献{' '}
              {data.retail.children.find((c: any) => c.key === 'KOL')?.pct || '0%'}，自然流量{' '}
              {data.retail.children.find((c: any) => c.key === 'Organic')?.pct || '0%'}，IB &
              Affiliate 贡献 {data.ib.formatPct}。
            </span>
          )}
        </div>
      </div>
