...
const CustomScatterNode = (props: any) => {
  ...
  const glow =
    payload.sentiment === 'negative' ? 'rgba(205, 92, 92, 0.15)' : 'rgba(113, 188, 136, 0.15)';
    ...
    return (
      <div className="bg-gray-900 text-white p-3 rounded-xl shadow-xl text-xs border border-gray-700 min-w-[150px]">
        ...
        <div className="flex justify-between mb-1">
          ...
          <span
            className={`font-bold ${data.sentiment === 'negative' ? 'text-rose-400' : 'text-emerald-400'}`}
          >
  ...
  return (
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 h-full flex flex-col">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-gray-500 uppercase tracking-widest font-semibold mt-0.5">
              Sentiment & AI Real-Time Monitor
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 min-h-[400px] w-full relative pt-6 pb-12 pl-[56px] pr-8 flex flex-col">
        ...
        <div className="absolute left-14 top-5 text-[9px] font-bold tracking-widest text-slate-400 uppercase pointer-events-none">
          High
        </div>
        <div className="absolute left-14 bottom-14 text-[9px] font-bold tracking-widest text-slate-400 uppercase pointer-events-none">
          Low
        </div>
        ...
        <div className="absolute left-[80px] right-[40px] bottom-2 h-[24px] flex items-center justify-between z-10 pointer-events-none">
          ...
          <div className="flex items-center gap-2.5 opacity-90">
            <span className="text-[11px] font-bold text-slate-700 tracking-widest">
              中性的{' '}
              <span className="text-[10px] uppercase text-slate-400 ml-0.5 tracking-wider">
                Neutral
              </span>
            </span>
      ...
      <div className="bg-[#1a1f2e] text-white p-5 rounded-2xl relative overflow-hidden flex flex-col md:flex-row gap-6 mt-2 shadow-lg transition-all duration-300">
        ...
        <div className="flex items-start gap-3 min-w-[200px]">
          ...
          <div>
            ...
            <div className="flex gap-2">
              <span className="text-[10px] bg-white/10 px-2 py-1 rounded text-gray-300 font-medium uppercase">
                {activeNode.platform}
              </span>
              <span className="text-[10px] bg-white/10 px-2 py-1 rounded text-gray-300 font-medium">
                声量: {(activeNode.z / 1000).toFixed(1)}K
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 md:border-l border-white/10 md:pl-6">
          ...
          <div className="text-sm text-gray-200 leading-relaxed">{activeNode.insight}</div>
        </div>
        ...
        <div className="flex-1 md:border-l border-white/10 md:pl-6">
          ...
          <div
            className={`text-xs p-3 rounded-lg leading-relaxed border ${
              activeNode.sentiment === 'negative'
