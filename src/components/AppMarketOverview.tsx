...
const AppMarketOverview = () => {
  ...
  useEffect(() => {
    ...
    const loadRealData = async () => {
      setLoading(true);
      try {
        const rawData = await fetchSensorTowerData(
          platform as any,
          selectedRegion,
          timeRange
        );
        if (active) {
          const processed = rawData.map(item => {
            ...
            const variance = selectedComp === 'Exness' ? 1.0 : selectedComp === 'TradeMaster' ? 0.8 : 0.7;
            
            return {
              ...item,
              displayReviews: `${(item.reviews / 1000).toFixed(1)}k`,
              compScore: hasComp ? Number((item.compScore ? item.compScore * variance : item.score * 0.95).toFixed(1)) : undefined,
              compDownloads: hasComp ? Math.round((item.compDownloads ? item.compDownloads * variance : item.downloads * 1.15)) : undefined,
              compReviews: hasComp ? Math.round((item.compReviews ? item.compReviews * variance : item.reviews * 1.2)) : undefined,
            };
          });
