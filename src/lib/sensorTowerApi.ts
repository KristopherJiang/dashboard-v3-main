...
export async function fetchSensorTowerData(
  ...
  try {
    ...
    if (isIOS && json.ios && json.ios.length > 0) {
      ...
      console.log(`[SensorTower client] Live downloads aggregated - April: Vantage=${vantageSumApril}, Exness=${exnessSumApril}. May: Vantage=${vantageSumMay}, Exness=${exnessSumMay}`);
    ...
    console.error("[SensorTower Client Error] Gracefully falling back to high-fidelity simulated models:", error);
