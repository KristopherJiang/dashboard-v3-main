...
const ChannelEfficiencyMatrix = () => {
  ...
  const hierarchicalData = useMemo(() => {
    ...
    const dsp_u = Math.round(paid_u * 0.2);
    ...
    return [
      ...
      },
      {
        ...
        level: 1,
        newUsers: retail_u,
        spend: retail_u * c(bSignup.retail),
        signup: c(bSignup.retail),
        kyc: c(28.0),
        ftd: c(98.0),
        ftt: c(132.0),
        roi: 508,
        ...
        children: [
          {
            ...
            level: 2,
            newUsers: kol_u,
            spend: kol_u * c(bSignup.kol),
            signup: c(bSignup.kol),
            kyc: c(18.0),
            ftd: c(65.0),
            ftt: c(82.0),
            roi: 720,
          ...
          },
          {
            ...
            level: 2,
            newUsers: paid_u,
            spend: paid_u * c(bSignup.paid),
            signup: c(bSignup.paid),
            kyc: c(38.0),
            ftd: c(120.0),
            ftt: c(155.0),
            roi: 450,
            ...
            children: [
              {
                ...
                level: 3,
                newUsers: dsp_u,
                spend: dsp_u * c(bSignup.dsp),
                signup: c(bSignup.dsp),
                kyc: c(30.0),
                ftd: c(95.0),
                ftt: c(125.0),
                roi: 480,
              ...
              },
              {
                ...
                level: 3,
                newUsers: asa_u,
                spend: asa_u * c(bSignup.asa),
                signup: c(bSignup.asa),
                kyc: c(42.0),
                ftd: c(145.0),
                ftt: c(195.0),
                roi: 320,
              ...
              },
              {
                ...
                level: 3,
                newUsers: google_u,
                spend: google_u * c(bSignup.google),
                signup: c(bSignup.google),
                kyc: c(25.0),
                ftd: c(85.0),
                ftt: c(105.0),
                roi: 557,
          ...
          },
          {
            ...
            level: 2,
            newUsers: organic_u,
            spend: organic_u * c(bSignup.organic),
            signup: c(bSignup.organic),
            kyc: c(8.5),
            ftd: c(35.0),
            ftt: c(48.0),
            roi: 1120,
          ...
          {
            id: 'raf',
            channel: 'RAF（Refer a Friend）',
            level: 2,
            newUsers: raf_u,
            spend: raf_u * c(bSignup.raf),
            signup: c(bSignup.raf),
            kyc: c(12.0),
            ftd: c(45.0),
            ftt: c(58.0),
            roi: 890,
  ...
  const flattenedData = useMemo(() => {
    ...
    const walk = (nodes: typeof hierarchicalData) => {
      nodes.forEach((node: (typeof hierarchicalData)[number]) => {
        result.push(node);
  ...
  // 3. 高级热力渐变函数
  const getHeatmapStyle = (val: number, type: string, columnMax: number, level: number) => {
        ...
        <table className="w-full text-left border-collapse min-w-[800px]">
              ...
              <th className="p-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest text-right">
                <div className="flex items-center justify-end gap-1 relative group">
                  ...
                  <div className="absolute top-full right-0 mt-2 hidden group-hover:block w-48 bg-gray-900 text-white text-[10px] p-2.5 rounded-lg shadow-xl z-50 normal-case tracking-normal text-left border border-gray-800">
                    <p className="text-gray-400 leading-relaxed mb-1.5">
                      获取单个新注册用户的平均花费。
                    </p>
                    <p className="text-indigo-300 font-mono bg-indigo-500/10 inline-block px-1.5 py-0.5 rounded">
                      总支出 / 新增注册数
                    </p>
          ...
          <tbody className="divide-y divide-slate-50">
            {flattenedData.map((row) => (
              <tr
                key={row.id}
                className={`group transition-all duration-300 ${row.level === 1 ? 'bg-slate-50/30' : 'hover:bg-slate-50/60'}`}
              >
                <td className="p-4">
                  <div
                    className="flex items-center gap-2"
                    style={{ paddingLeft: `${(row.level - 1) * 24}px` }}
                  >
                    {row.children ? (
                      <button
                        onClick={() => toggleRow(row.id)}
                        className={`w-5 h-5 rounded flex items-center justify-center shadow-sm transition-all ${expandedRows.includes(row.id) ? 'bg-white text-slate-600 border border-slate-200' : 'bg-emerald-400 text-white shadow-emerald-400/20'}`}
                      >
                    ...
                    <span
                      className={`${row.level === 1 ? 'font-bold text-slate-900 text-sm' : 'font-semibold text-slate-700 text-xs'} truncate`}
                    >
                      {row.channel}
                    </span>
                  </div>
                </td>
                <td className="p-4 text-right font-bold text-slate-700 text-xs tabular-nums">
                  {row.newUsers.toLocaleString()}
                </td>
                <td className="p-4 text-right font-semibold text-slate-400 text-xs tabular-nums">
                  ${row.spend.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                </td>
                {/* 成本列：数值越大，红色越深 */}
                <td
                  className="p-4 text-right font-bold text-xs tabular-nums transition-colors duration-500"
                  style={getHeatmapStyle(row.signup, 'cost', maxValues.signup, row.level)}
                >
                  ${row.signup.toFixed(1)}
                </td>
                <td
                  className="p-4 text-right font-bold text-xs tabular-nums transition-colors duration-500"
                  style={getHeatmapStyle(row.kyc, 'cost', maxValues.kyc, row.level)}
                >
                  ${row.kyc.toFixed(1)}
                </td>
                <td
                  className="p-4 text-right font-bold text-xs tabular-nums transition-colors duration-500"
                  style={getHeatmapStyle(row.ftd, 'cost', maxValues.ftd, row.level)}
                >
                  ${row.ftd.toFixed(1)}
                </td>
                <td
                  className="p-4 text-right font-bold text-xs tabular-nums transition-colors duration-500"
                  style={getHeatmapStyle(row.ftt, 'cost', maxValues.ftt, row.level)}
                >
                  ${row.ftt.toFixed(1)}
                </td>
                {/* LTV/CAC 列：数值越大，绿色越深 */}
                <td
                  className="p-4 text-right font-bold text-xs transition-colors duration-500"
                  style={getHeatmapStyle(row.ltv, 'profit', maxValues.ltv, row.level)}
                >
                  <span
                    className={`${row.ltv > 5 ? 'text-emerald-400' : row.ltv > 3 ? 'text-slate-700' : 'text-rose-600'}`}
                  >
                    {row.ltv}x
                  </span>
                </td>
      ...
      <div className="px-6 pb-6 pt-2 grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-emerald-50/50 p-5 rounded-2xl border border-emerald-100/50 flex flex-col justify-between h-full group hover:shadow-lg hover:shadow-emerald-400/5 transition-all duration-300">
          <div>
            ...
            <p className="text-slate-500 text-xs leading-relaxed font-medium">
              LTV/CAC 高达 <span className="text-emerald-400 font-bold">12.4x</span>
              ，且获客成本仅为大盘均值的 20%。建议强化内容深度，沉淀长效私域流量。
            </p>
          </div>
        </div>
        ...
        <div className="bg-rose-50/50 p-5 rounded-2xl border border-rose-100/50 flex flex-col justify-between h-full group hover:shadow-lg hover:shadow-rose-500/5 transition-all duration-300">
          <div>
            ...
            <p className="text-slate-500 text-xs leading-relaxed font-medium">
              FTT 转化成本达 <span className="text-rose-600 font-bold">$195.0</span>
              ，出现成本倒挂迹象。需紧急排查关键词精准度与首屏流失率。
            </p>
          </div>
        </div>
        ...
        <div className="bg-blue-50/50 p-5 rounded-2xl border border-blue-100/50 flex flex-col justify-between h-full group hover:shadow-lg hover:shadow-blue-500/5 transition-all duration-300">
          <div>
            ...
            <p className="text-slate-500 text-xs leading-relaxed font-medium">
              当前营销组合整体健康。FTT 是提升 ROI
              的最大杠杆，建议优化支付链路，减少注册后的首笔转化摩擦。
            </p>
          </div>
        </div>
      </div>
