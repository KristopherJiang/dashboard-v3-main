...
const TrendBadge = ({ change }: { change: number }) => {
  ...
  return (
    <div
      className={`flex items-center gap-0.5 text-[10px] font-bold ${isPositive ? 'text-emerald-400' : 'text-rose-500'}`}
    >
      {isPositive ? (
...
const MiniCard = ({ label, value, change }: { label: string; value: number; change: number }) => (
  <div className="bg-white border border-slate-200/70 p-3 rounded-xl shadow-sm hover:shadow-md hover:border-blue-100 transition-all duration-300">
    <div className="text-slate-400 text-[9px] font-black mb-1.5 uppercase tracking-widest">
      {label}
    </div>
  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-[#1a1f2e] border border-slate-700 p-2.5 rounded-lg shadow-2xl">
        <p className="text-slate-400 text-[9px] font-bold mb-1.5 border-b border-slate-700/50 pb-1.5 uppercase tracking-wider">
          {label} 数据详情
        </p>
          ...
          {payload.map((entry: any, index: number) => (
            <div key={index} className="flex items-center justify-between gap-4">
              ...
              <span className="text-white font-bold text-[10px]">
                {Number(entry.value).toFixed(1)}k
              </span>
            </div>
          ))}
        </div>
...
const GlobalIntelligence = () => {
  ...
  const regionBucket =
    selectedRegion === 'GLOBAL'
      ? 'Global'
      : selectedRegion.startsWith('ASIA')
        ? 'Asia'
        : selectedRegion.startsWith('EU')
          ? 'EU'
          : selectedRegion.startsWith('LATAM')
            ? 'Latam'
            : selectedRegion.startsWith('MENA')
              ? 'Mena'
              : selectedRegion.startsWith('AFRICA')
                ? 'Africa'
                : 'Others';
  ...
  const baseSeoData: Record<string, any> = {
    ...
    Mena: {
      kpis: {
        authorityScore: { value: 75, change: 1, status: '稳定', percentile: '70%' },
        organicTraffic: { value: 8.2, change: 4.1 },
        organicKeywords: { value: 6.5, change: 3.2 },
        paidTraffic: { value: 2.1, change: 0.5 },
        paidKeywords: { value: 1.2, change: 0.2 },
      },
      chart: Array.from({ length: 12 }, (_, i) => ({
        month: `${i + 1}月`,
        brand: 5 + i,
    ...
    EU: {
      kpis: {
        authorityScore: { value: 85, change: -1, status: '良好', percentile: '82%' },
        organicTraffic: { value: 22.1, change: -2.1 },
        organicKeywords: { value: 18.5, change: 1.1 },
        paidTraffic: { value: 4.5, change: -5.5 },
        paidKeywords: { value: 3.2, change: -2.4 },
      },
      chart: Array.from({ length: 12 }, (_, i) => ({
        month: `${i + 1}月`,
        brand: 20 + i,
    ...
    Latam: {
      kpis: {
        authorityScore: { value: 70, change: 3, status: '上升', percentile: '65%' },
        organicTraffic: { value: 5.4, change: 12.4 },
        organicKeywords: { value: 4.1, change: 8.2 },
        paidTraffic: { value: 1.2, change: 2.5 },
        paidKeywords: { value: 0.8, change: 1.4 },
      },
      chart: Array.from({ length: 12 }, (_, i) => ({
        month: `${i + 1}月`,
        brand: 4 + i,
    ...
    Africa: {
      kpis: {
        authorityScore: { value: 62, change: 0.5, status: '潜力', percentile: '55%' },
        organicTraffic: { value: 2.1, change: 1.2 },
        organicKeywords: { value: 1.8, change: 0.9 },
        paidTraffic: { value: 0.5, change: -0.2 },
        paidKeywords: { value: 0.3, change: -0.1 },
      },
      chart: Array.from({ length: 12 }, (_, i) => ({
        month: `${i + 1}月`,
        brand: 2 + i,
    ...
    Others: {
      kpis: {
        authorityScore: { value: 55, change: 0.2, status: '平稳', percentile: '45%' },
        organicTraffic: { value: 1.5, change: 0.5 },
        organicKeywords: { value: 1.2, change: 0.3 },
        paidTraffic: { value: 0.2, change: -0.1 },
        paidKeywords: { value: 0.1, change: 0 },
      },
      chart: Array.from({ length: 12 }, (_, i) => ({
        month: `${i + 1}月`,
        brand: 1 + i,
  ...
  const currentSeoData = {
    kpis: {
      ...
      organicTraffic: {
        value: Math.round(rawSeoData.kpis.organicTraffic.value * m * 10) / 10,
        change: rawSeoData.kpis.organicTraffic.change,
      },
      organicKeywords: {
        value: Math.round(rawSeoData.kpis.organicKeywords.value * m * 10) / 10,
        change: rawSeoData.kpis.organicKeywords.change,
      },
      paidTraffic: {
        value: Math.round(rawSeoData.kpis.paidTraffic.value * m * 10) / 10,
        change: rawSeoData.kpis.paidTraffic.change,
      },
      paidKeywords: {
        value: Math.round(rawSeoData.kpis.paidKeywords.value * m * 10) / 10,
        change: rawSeoData.kpis.paidKeywords.change,
      },
    },
  ...
  const compTrend =
    selectedBrand !== 'Vantage'
      ? visibilityTrendData[selectedBrand as keyof typeof visibilityTrendData]
      : null;
        ...
        {activeTab === 'SEO' && (
          <div className="flex-1 flex flex-col gap-4 animate-in fade-in duration-300 overflow-y-auto pr-2 pb-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-bold text-slate-500 tracking-tight">
                    ...
                    <span className="text-slate-900">
                      {currentSeoData.kpis.authorityScore.percentile}
                    </span>{' '}
                    同类站点
                  </span>
                </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <MiniCard
                label="自然流量"
                value={currentSeoData.kpis.organicTraffic.value}
                change={currentSeoData.kpis.organicTraffic.change}
              />
              <MiniCard
                label="自然关键词"
                value={currentSeoData.kpis.organicKeywords.value}
                change={currentSeoData.kpis.organicKeywords.change}
              />
              <MiniCard
                label="付费流量"
                value={currentSeoData.kpis.paidTraffic.value}
                change={currentSeoData.kpis.paidTraffic.change}
              />
              <MiniCard
                label="付费关键词"
                value={currentSeoData.kpis.paidKeywords.value}
                change={currentSeoData.kpis.paidKeywords.change}
              />
            </div>
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={idx} className="grid grid-cols-3 gap-2 items-center">
                    <div className="flex items-center gap-2">
                      <div
                        className={`w-5 h-5 rounded-md ${item.bg} flex items-center justify-center shrink-0`}
                      >
                        ...
                        if (props.active && props.payload && props.payload.length) {
                          return (
                            <div className="bg-white p-3 rounded-xl shadow-xl border border-gray-100 min-w-[120px]">
                              <div className="text-[10px] font-bold text-slate-400 mb-2 pb-1 border-b border-slate-50 uppercase tracking-widest">
                                {props.label} 分数
                              </div>
                ...
                {brands.map((brand, idx) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <span
                        className={`text-xs font-bold ${selectedBrand === brand.name ? 'text-gray-900' : 'text-gray-600'}`}
                      >
                        {brand.name}
                      </span>
                    </div>
                    <div className="flex items-center gap-3">
                      <span
                        className={`text-[10px] font-bold flex items-center gap-0.5 ${brand.changeType === 'up' ? 'text-emerald-400' : 'text-rose-500'}`}
                      >
                        {brand.changeType === 'up' ? (
                          <TrendingUp size={10} />
                        ) : (
