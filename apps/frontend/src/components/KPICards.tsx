	...
	if (active && payload && payload.length) {
		...
		return (
			<div className="bg-gray-900 text-white p-3 rounded-xl shadow-xl text-[11px] border border-gray-700 min-w-[140px]">
				<div className="text-gray-400 mb-2 font-bold border-b border-gray-800 pb-1.5">
					{title}
				</div>
...
const NetDepositChart = ({
	...
	const rawMonthData = [
		{
			deposit: 4500,
			withdrawal: 2000,
			depositLastYear: 3800,
			withdrawalLastYear: 1800,
		},
		{
			deposit: 5200,
			withdrawal: 1800,
			depositLastYear: 4500,
			withdrawalLastYear: 1500,
		},
		{
			deposit: 4800,
			withdrawal: 3000,
			depositLastYear: 4200,
			withdrawalLastYear: 2800,
		},
		{
			deposit: 7000,
			withdrawal: 2500,
			depositLastYear: 6000,
			withdrawalLastYear: 2200,
		},
		{
			deposit: 8500,
			withdrawal: 4000,
			depositLastYear: 7500,
			withdrawalLastYear: 3500,
		},
		{
			deposit: 9500,
			withdrawal: 3500,
			depositLastYear: 8000,
			withdrawalLastYear: 3200,
		},
		{
			deposit: 10000,
			withdrawal: 5000,
			depositLastYear: 9000,
			withdrawalLastYear: 4800,
		},
		{
			deposit: 11500,
			withdrawal: 6000,
			depositLastYear: 10000,
			withdrawalLastYear: 5500,
		},
		{
			deposit: 13400,
			withdrawal: 4500,
			depositLastYear: 12000,
			withdrawalLastYear: 4200,
		},
		{
			deposit: 11000,
			withdrawal: 5500,
			depositLastYear: 9500,
			withdrawalLastYear: 5800,
		},
		{
			deposit: 11500,
			withdrawal: 4000,
			depositLastYear: 10500,
			withdrawalLastYear: 3800,
		},
		{
			deposit: 12500,
			withdrawal: 4800,
			depositLastYear: 11000,
			withdrawalLastYear: 4500,
		},
	];
	...
	const maxVal =
		Math.max(
			...monthDataWithLabels.map((d) => Math.max(d.deposit, d.withdrawal)),
		) * 1.1;
						...
						<g key={i} className="transition-all duration-300">
							...
							<rect
								x={
									width / 2 -
									20 -
									((d.withdrawalLastYear || 0) / maxVal) * chartWidth
								}
								y={yPos - barHeight / 2}
			...
			{activeIndex !== null && (
				...
				>
					<div className="text-[10px] font-black text-gray-400 mb-2 border-b border-gray-800 pb-1.5">
						{monthDataWithLabels[activeIndex].fullRange ||
							monthDataWithLabels[activeIndex].label}
					</div>
...
function chartDataToTrendRetention(
	points: { month: string; current: number; previous: number }[],
) {
	return points.map((p) => ({
		month: p.month,
...
function chartDataToConversion2(
	points: { month: string; current: number; previous: number }[],
) {
	return points.map((p) => ({
		month: p.month,
...
function getTrendFromCard(
	...
	const pop =
		typeof card.trendPoP === "number"
			? card.trendPoP
			: Number(card.trendPoP) || 0;
	const yoy =
		typeof card.trendYoY === "number"
			? card.trendYoY
			: Number(card.trendYoY) || 0;
	return {
		timeCtx,
...
export default function KPICards() {
	...
	const fallbackA2 = React.useMemo(
		() => remapChartData(dataA2),
		[remapChartData],
	);
	const fallbackB = React.useMemo(
		() => remapChartData(dataB),
		[remapChartData],
	);
	const fallbackC = React.useMemo(
		() => remapChartData(dataC),
		[remapChartData],
	);
	const fallbackConv1 = React.useMemo(
		() => remapChartData(dataConversion1),
		[remapChartData],
	);
	const fallbackConv2 = React.useMemo(
		() => remapChartData(dataConversion2),
		[remapChartData],
	);
	...
	const trendA = regCard
		? getTrendFromCard(regCard, timeCtx)
		: getTrendConfig(timeCtx, 9.4);
	const trendFTD = ftdCard
		? getTrendFromCard(ftdCard, timeCtx)
		: getTrendConfig(timeCtx, 12.2);
	const trendFTT = fttCard
		? getTrendFromCard(fttCard, timeCtx)
		: getTrendConfig(timeCtx, 15.6);
	const trendDep = depCard
		? getTrendFromCard(depCard, timeCtx)
		: getTrendConfig(timeCtx, 23.3);
	const trendVol = volCard
		? getTrendFromCard(volCard, timeCtx)
		: getTrendConfig(timeCtx, 25.6);
	const trendConv1 = cv1Card
		? getTrendFromCard(cv1Card, timeCtx)
		: getTrendConfig(timeCtx, 2.4);
	const trendConv2 = cv2Card
		? getTrendFromCard(cv2Card, timeCtx)
		: getTrendConfig(timeCtx, 1.8);
	const trendRet = retCard
		? getTrendFromCard(retCard, timeCtx)
		: getTrendConfig(timeCtx, 1.2);

	// 核心数值：从 API 读取或 fallback（API 有数据时 fallback 不生效）
	const fallbackMultiplier = 1;
	const regVal = regCard
		? Number(regCard.value).toLocaleString()
		: Math.floor(12480 * fallbackMultiplier).toLocaleString();
	const ftdVal = ftdCard
		? Number(ftdCard.value).toLocaleString()
		: Math.floor(2995 * fallbackMultiplier).toLocaleString();
	const fttVal = fttCard
		? Number(fttCard.value).toLocaleString()
		: Math.floor(1497 * fallbackMultiplier).toLocaleString();
	const depVal = depCard
		? `$${Number(depCard.value).toLocaleString()}`
		: `$${Math.floor(1542060 * fallbackMultiplier).toLocaleString()}`;
	const volVal = volCard
		? `$${Number(volCard.value).toLocaleString()}`
		: `$${Math.floor(15420600 * fallbackMultiplier).toLocaleString()}`;
