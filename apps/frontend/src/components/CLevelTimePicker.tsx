...
import type { TimeRange } from "../lib/DashboardContext";
...
const CLevelTimePicker = () => {
	...
	const {
		timeRange: selectedId,
		setTimeRange: setSelectedId,
		customDateRange,
		setCustomDateRange,
	} = useDashboardContext();
	...
	const isRangeBound = (day: number) => {
		if (selectedId !== "custom" || !selectionStart || !selectionEnd)
			return false;
		const d = new Date(year, month, day);
		return (
			d.getTime() === selectionStart.getTime() ||
			d.getTime() === selectionEnd.getTime()
		);
	};
						...
						<div className="w-[180px] bg-slate-50/50 border-r border-gray-100 p-3 space-y-1">
							...
							{PRESETS.map((p) => (
								<button
									...
									key={p.id}
									onClick={() => {
										setSelectedId(p.id as TimeRange);
										setIsOpen(false);
									}}
								...
								>
									<div className="flex flex-col items-start leading-none">
										...
										<span
											className={`text-[8px] uppercase tracking-tighter mt-1 ${selectedId === p.id ? "text-gray-400" : "text-gray-400"}`}
										>
											{p.sub}
										</span>
									</div>
						...
						<div className="flex-1 p-6 flex flex-col">
							...
							<div className="grid grid-cols-7 gap-1 text-center mb-6">
								{["一", "二", "三", "四", "五", "六", "日"].map((d) => (
									<span
										key={d}
										className="text-[10px] font-bold text-gray-300 py-1"
									>
										{d}
									</span>
								))}
							...
							<div className="mt-auto pt-5 border-t border-gray-100 flex items-center justify-between">
								...
								>
									<div
										className={`absolute top-1 left-1 w-3 h-3 bg-white rounded-full transition-transform ${isCompareMode ? "translate-x-5" : ""}`}
									></div>
								</button>
							</div>
